...
class InvoiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(Auth::user()->can('invoice create'))
        {
            if($request->invoice_type == "product")
            {
                    ...
                    //slack
                    if(module_is_active('Slack') && !empty(company_setting('New Invoice')) && company_setting('New Invoice')  == true)
                    {
                        $msg = "New invoice" ." ". Invoice::invoiceNumberFormat($invoice->invoice_id)." created by ". Auth::user()->name.'.';

                        // SendSlackMsg
                        event(new \Modules\Slack\Events\SendSlackMsg($msg));
                    }
                    //telegram
                    if(module_is_active('Telegram') && !empty(company_setting('Telegram New Invoice')) && company_setting('Telegram New Invoice')  == true)
                    {
                        $msg = "New invoice" ." ". Invoice::invoiceNumberFormat($invoice->invoice_id)." created by ". Auth::user()->name.'.';
                        // SendTelegramMsg
                    ...
                    //twilio
                    if(module_is_active('Twilio') && !empty(company_setting('Twilio New Invoice')) && company_setting('Twilio New Invoice')  == true)
                    {
                    ...
                    if(module_is_active('Webhook')){
                        ...
                        if($invoice->user_id){
                            ...
                            $invoice->customer_id = $customer->name;
                        }
                        if($invoice->category_id){
                            $category = \Modules\ProductService\Entities\Category::where('id',$invoice->category_id)->where('type', 1)->first();
                            $invoice->category_id = $category->name;
                        }
                        unset($invoice->user_id);
                        $action = 'New Invoice';
                        $module = 'general';
                        event(new \Modules\Webhook\Events\SendWebhook($module ,$invoice,$action));
                    }
    ...
    public function sent($id)
    {
        if(Auth::user()->can('invoice send'))
        {
            ...
            //slack
            if(module_is_active('Slack') && !empty(company_setting('Invoice status updated')) && company_setting('Invoice status updated')  == true)
            {
            ...
            ///telegram
            if(module_is_active('Telegram') && !empty(company_setting('Telegram Invoice status updated')) && company_setting('Telegram Invoice status updated')  == true)
            {
                $msg = " Invoice status updated by ". Auth::user()->name.'.';
                // SendTelegramMsg
                event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
            ...
            //twilio
            if(module_is_active('Twilio') && !empty(company_setting('Twilio Invoice status updated')) && company_setting('Twilio Invoice status updated')  == true)
            {
                if(module_is_active('Account'))
                {
        ...
        if(Auth::user()->can('invoice payment create'))
        {
            ...
            if(module_is_active('Twilio') && !empty(company_setting('Twilio Invoice status updated')) && company_setting('Twilio Invoice status updated')  == true)
            {
                if(!empty($customer->mobile_no)){
                    $msg = " Invoice status updated by ". Auth::user()->name.'.';
                    event(new \Modules\Twilio\Events\SendTwilioMsg($customer->mobile_no,$msg));
                }
            }
               //Email notification
