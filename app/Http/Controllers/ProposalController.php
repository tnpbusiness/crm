...
class ProposalController extends Controller
{
        ...
        if(module_is_active('ProductService'))
        {
            if (Auth::user()->can('proposal create'))
            {
                ...
                if(module_is_active('Account'))
                {
                    ...
                    $category = \Modules\ProductService\Entities\Category::where('workspace_id', getActiveWorkSpace())->where('type', 1)->get()->pluck
                    ('name', 'id');
        ...
        if (\Auth::user()->can('proposal create'))
        {
            if($request->proposal_type == "product")
            {
                ...
                for ($i = 0; $i < count($products); $i++)
                {
                 ...
                 //slack
                if(module_is_active('Slack') && !empty(company_setting('New Proposal')) && company_setting('New Proposal')  == true)
                {
                    $msg = "New Proposal created by ". Auth::user()->name.'.';

                    // SendSlackMsg
                    event(new \Modules\Slack\Events\SendSlackMsg($msg));
                }
                // telegram
                if(module_is_active('Telegram') && !empty(company_setting('Telegram New Proposal')) && company_setting('Telegram New Proposal')  == true)
                {
                    $msg = "New Proposal created by ". Auth::user()->name.'.';
                    // SendTelegramMsg
                ...
                //twilio
                if(module_is_active('Twilio') && !empty(company_setting('Twilio New Proposal')) && company_setting('Twilio New Proposal')  == true)
                {
                    $user =User::where('id',$request->customer_id)->first();
                    if(!empty($user->mobile_no)){
                        $msg = "New Proposal created by ". Auth::user()->name.'.';
                        event(new \Modules\Twilio\Events\SendTwilioMsg($user->mobile_no,$msg));
                    }
                }
                //webhook
                if(module_is_active('Webhook')){
                    $action = 'New Proposal';
                    $module = 'general';
                    event(new \Modules\Webhook\Events\SendWebhook($module ,$proposal,$action));
                }
    ...
    public function statusChange(Request $request, $id)
    {
        ...
        //slack
        if(module_is_active('Slack') && !empty(company_setting('Proposal Status Updated')) && company_setting('Proposal Status Updated')  == true)
        {
            $msg = " Proposal status updated by ". Auth::user()->name.'.';

            // SendSlackMsg
            event(new \Modules\Slack\Events\SendSlackMsg($msg));
        }
        ///telegram
        if(module_is_active('Telegram') && !empty(company_setting('Telegram Proposal status updated')) && company_setting('Telegram Proposal status updated')  == true)
        {
        $msg = " Proposal status updated by ". Auth::user()->name.'.';
            // SendTelegramMsg
            event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
        ...
        // twilio
        if(module_is_active('Twilio') && !empty(company_setting('Twilio Proposal status updated')) && company_setting('Twilio Proposal status updated')  == true)
        {
            $Assign_user_phone = User::where('id',$proposal->customer_id)->first();
        ...
        if (\Auth::user()->can('proposal send'))
        {
            ...
            if(module_is_active('Twilio') && !empty(company_setting('Twilio Proposal status updated')) && company_setting('Twilio Proposal status updated')  == true)
            {
                ...
                if(!empty($number)){
                    ...
                    event(new \Modules\Twilio\Events\SendTwilioMsg($number,$msg));
                }
            }
            //webhook
            if(module_is_active('Webhook')){
                $proposal_status = new Proposal;
                $proposal_status->id = $proposal->id;
                $proposal_status->send_date = $proposal->send_date;
                $proposal_status->status    = Proposal::$statues[$proposal->status];
                $action = 'Proposal status updated';
                $module = 'general';
                event(new \Modules\Webhook\Events\SendWebhook($module ,$proposal_status,$action));
            }
            //Email notification
            if(!empty(company_setting('Proposal Status Updated')) && company_setting('Proposal Status Updated')  == true)
            {
