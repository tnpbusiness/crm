...
class PaymentController extends Controller
{
    ...
    public function create()
    {
        if(\Auth::user()->can('expense payment create'))
        {
            ...
            if(module_is_active('ProductService'))
            {
                $categories = \Modules\ProductService\Entities\Category::where('workspace_id', getActiveWorkSpace())->where('type','2')->get()->pluck('name', 'id');
            }
    ...
    public function store(Request $request)
    {
        if(Auth::user()->can('expense payment create'))
        {
            ...
            //slack
            if(module_is_active('Slack') && !empty(company_setting('New Payment')) && company_setting('New Payment')  == true)
            {
                $msg = __("New payment of").' ' . currency_format_with_sym($request->amount) . __("created for").' ' . $vender->name . __("by").' '.  $payment->type . '.';
                // SendSlackMsg
                event(new \Modules\Slack\Events\SendSlackMsg($msg));
            }
            //telegram
            if(module_is_active('Telegram') && !empty(company_setting('Telegram New Payment')) && company_setting('Telegram New Payment')  == true)
            {
                $msg = __("New payment of").' ' . currency_format_with_sym($request->amount) . __("created for").' ' . $vender->name . __("by").' '.  $payment->type . '.';
                // SendTelegramMsg
          ...
          //twilio
          if(module_is_active('Twilio') && !empty(company_setting('Twilio New Payment')) && company_setting('Twilio New Payment')  == true)
          {
              if(!empty($vender->contact)){
                $msg = __("New payment of").' ' . currency_format_with_sym($request->amount) . __("created for").' ' . $vender->name . __("by").' '.  $payment->type . '.';
                  event(new \Modules\Twilio\Events\SendTwilioMsg($vender->contact,$msg));
              }
          }
          if(!empty($vendor))
            {
            ...
            if(module_is_active('Webhook')){
                ...
                if($account)
                {
                    $payments->account_id = $account->bank_name;
                }
                ...
                $categories = \Modules\ProductService\Entities\Category::where('id', $request->category_id)->where('type', 2)->first();
                if($categories)
                {
                    $payments->category_id = $categories->name;
                }
                $payments->vendor_id = $customer->name;
                $action = 'New Payment';
                $module = 'Account';
                event(new \Modules\Webhook\Events\SendWebhook($module ,$payments,$action));
            }
    ...
    {
        return redirect()->route('payment.index')->with('error', __('Permission denied.'));
