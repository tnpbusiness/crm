...
<div>
    @can('leave create')
        <a  class="btn btn-sm btn-primary" data-ajax-popup="true" data-size="md" data-title="{{ __('Create New Leave') }}" data-url="{{route('leave.create')}}" data-toggle="tooltip" title="{{ __('Create') }}">
                ...
                <div class="table-responsive">
                    <table class="table mb-0 pc-dt-simple" id="assets">
                        ...
                        <tbody>
                            ...
                            <tr>
                                @if (in_array(\Auth::user()->type, \Auth::user()->not_emp_type))
                                    <td>{{ !empty( Modules\Hrm\Entities\Employee::getEmployee($leave->user_id)) ? Modules\Hrm\Entities\Employee::getEmployee($leave->user_id)->name : '--' }}</td>
                                @endif
                                        ...
                                        <span>
                                            ...
                                            @if ($leave->status == 'Pending')
                                                ...
                                                <div class="action-btn bg-danger ms-2">
                                                        {{Form::open(array('route'=>array('leave.destroy', $leave->id),'class' => 'm-0'))}}
                                                        @method('DELETE')
                                                            <a
                                                                class="mx-3 btn btn-sm  align-items-center bs-pass-para show_confirm"
                                                                data-bs-toggle="tooltip" title="" data-bs-original-title="Delete"
                                                                aria-label="Delete" data-confirm="{{__('Are You Sure?')}}" data-text="{{__('This action can not be undone. Do you want to continue?')}}"  data-confirm-yes="delete-form-{{$leave->id}}"><i
