    ...
    public function store(Request $request)
    {
        if (Auth::user()->can('holiday create'))
        {
            ...
            if(module_is_active('Calender'))
            {
                ...
                {
                    $type ='holiday';
                    $holiday->title=$request->occasion;
                    event(new \Modules\Calender\Events\GoogleCalender($holiday,$type));
                }
            }
            // // slack
            if(module_is_active('Slack') && !empty(company_setting('New Holidays')) && company_setting('New Holidays')  == true)
            {
            ...
            //telegram
            if(module_is_active('Telegram') && !empty(company_setting('Telegram New Holidays')) && company_setting('Telegram New Holidays')  == true)
            {
                $msg = $request->occasion . ' ' . __("on") . ' ' . $request->date . '.';
                // SendTelegramMsg
            ...
            //twilio
            if(module_is_active('Twilio') && !empty(company_setting('Twilio New Holidays')) && company_setting('Twilio New Holidays')  == true)
            {
                $msg = $request->occasion . ' ' . __("on") . ' ' . $request->date . '.';
            ...
            if(module_is_active('Webhook')){
                $action = 'New Holidays';
                $module = 'Hrm';
                event(new \Modules\Webhook\Events\SendWebhook($module ,$holiday,$action));
            }
