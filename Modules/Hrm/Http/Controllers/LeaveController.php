...
class LeaveController extends Controller
{
        ...
        if(Auth::user()->can('leave create'))
        {
            ...
            if($leave_type->days >= $total_leave_days)
            {
                ...
                if(module_is_active('Calender'))
                {
                    ...
                    {
                        $type ='leave';
                        $leave->title=$request->leave_reason;
                        event(new \Modules\Calender\Events\GoogleCalender($leave,$type));
                    }
                }
    ...
    public function changeaction(Request $request)
    {
        if(Auth::user()->can('leave approver manage'))
        {
            ...
            $leave->save();
            //twilio
            if(module_is_active('Twilio') && !empty(company_setting('Leave Approve/Reject')) && company_setting('Leave Approve/Reject')  == true)
            {
                $employee = Employee::where('id', '=', $leave->employee_id)->first();
                if(!empty($employee->phone)){
                    $msg = __("Your leave has been").' '.$leave->status.'.';
                    event(new \Modules\Twilio\Events\SendTwilioMsg($employee->phone,$msg));
                }
            }
