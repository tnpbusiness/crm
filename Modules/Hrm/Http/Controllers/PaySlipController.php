    ...
    public function store(Request $request)
    {
        ...
        if ($payslip_employee > count($validatePaysilp))
        {
            ...
            foreach ($employees as $employee)
            {
                ...
                // slack
                if(module_is_active('Slack') && !empty(company_setting('New Monthly Payslip')) && company_setting('New Monthly Payslip')  == true)
                {
                ...
                //telegram
                if(module_is_active('Telegram') && !empty(company_setting('Telegram New Monthly Payslip')) && company_setting('Telegram New Monthly Payslip')  == true)
                {
                    $month = date('M Y', strtotime($payslipEmployee->salary_month . ' ' . $payslipEmployee->time));
                    $msg = ("payslip generated of") . ' ' . $month . '.';
                    // SendTelegramMsg
                 ...
                 //twilio
                if(module_is_active('Twilio') && !empty(company_setting('Twilio New Monthly Payslip')) && company_setting('Twilio New Monthly Payslip')  == true)
                {
            ...
            if(module_is_active('Webhook')){
                ...
                $module = 'Hrm';
                event(new \Modules\Webhook\Events\SendWebhook($module ,$payslipEmployee,$action));
            }
