    ...
    */
    ...
    public function store(Request $request)
    {
        if(Auth::user()->can('project create'))
        {
            ...
            //slack
            if(module_is_active('Slack') && !empty(company_setting('New Project')) && company_setting('New Project')  == true)
            {
            ...
            // telegram
            if(module_is_active('Telegram') && !empty(company_setting('Telegram New Project')) && company_setting('Telegram New Project')  == true)
            {
                $msg =  $objProject->name ." created by ". Auth::user()->name.'.';
                // SendTelegramMsg
            ...
            // twilio
            if(module_is_active('Twilio') && !empty(company_setting('Twilio New Project')) && company_setting('Twilio New Project')  == true)
            {
                $projects = \Modules\Taskly\Entities\UserProject::where('project_id',$objProject->id)->get()->pluck('user_id');
            ...
            if(module_is_active('Webhook')){
                $objProject->users_list = User::whereIN('email', $userList)->get()->pluck('name')->toArray();
                $action = 'New Project';
                $module = 'Taskly';
                $status = event(new \Modules\Webhook\Events\SendWebhook($module ,$objProject,$action));
                if($status == false)
                {
    ...
    public function milestoneStore( $projectID, Request $request)
    {
        ...
        //slack
        if(module_is_active('Slack') && !empty(company_setting('    ')) && company_setting('New Milestone')  == true)
        {
            $msg = "New Milestone created by the ".Auth::user()->name.'.';
        ...
        // telegram
        if(module_is_active('Telegram') && !empty(company_setting('Telegram New Milestone')) && company_setting('Telegram New Milestone')  == true)
        {
            $msg = "New Milestone created by the ". Auth::user()->name.'.';
            // SendTelegramMsg
            event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
        ...
        if(module_is_active('Webhook')){
            ...
            $module = 'Taskly';
            event(new \Modules\Webhook\Events\SendWebhook($module ,$milestone,$action));
        }
            ...
            if($stage)
            {
                ...
                if(module_is_active('Calender'))
                {
                    if($request->get('synchronize_type')  == 'google_calender')
                    {
                        ...
                        event(new \Modules\Calender\Events\GoogleCalender($task,$type));
                    }
                }
                //slack
                if(module_is_active('Slack') && !empty(company_setting('New Task')) && company_setting('New Task')  == true)
                {
                ...
                // telegram
                if(module_is_active('Telegram') && !empty(company_setting('Telegram New Task')) && company_setting('Telegram New Task')  == true)
                {
                    $msg = $request->title . " of " . $project_name->name . " created by " . Auth::user()->name . '.';
                    // SendTelegramMsg
                    event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
                ...
                // twilio
                if(module_is_active('Twilio') && !empty(company_setting('Twilio New Task')) && company_setting('Twilio New Task')  == true)
                {
                    $Assign_user_phones = User::whereIn('id',$request->assign_to)->get();
                    foreach($Assign_user_phones as $Assign_user_phone){
                        if(!empty($Assign_user_phone->mobile_no))
                        {
                            $msg = $request->title . " of " . $project_name->name . " created by " . Auth::user()->name . '.';
                ...
                if(module_is_active('Webhook'))
                {
                    ...
                    $task->milestone_id = !empty($milestone)?$milestone->title:'';
                    $task->assign_to = (count($assign_user) > 0 ) ? implode(',',$assign_user) : 'Not Found';
                    $action = 'New Task';
                    $module = 'Taskly';
                    event(new \Modules\Webhook\Events\SendWebhook($module ,$task,$action));
                }
    ...
    public function taskOrderUpdate(Request $request, $projectID)
    {
        ...
        if($request->new_status != $request->old_status)
        {
            ...
            //slack
            if(module_is_active('Slack') && !empty(company_setting('Task Stage Updated')) && company_setting('Task Stage Updated')  == true)
            {
            ...
            // telegram
            if(module_is_active('Telegram') && !empty(company_setting('Telegram Task Stage Updated')) && company_setting('Telegram Task Stage Updated')  == true)
            {
                $msg = $task->title . " status changed from  " . $old_status->name . " to " . $new_status->name . '.';
                // SendTelegramMsg
                event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
            ...
            if(module_is_active('Webhook'))
            {
                ...
                $task_status->task_id = $task->title;
                $task_status->status = $new_status->name;
                $action = 'Task Stage Update';
                $module = 'Taskly';
                event(new \Modules\Webhook\Events\SendWebhook($module ,$task_status,$action));
            }
    ...
    public function commentStore(Request $request, $projectID, $taskID, $clientID = '')
    {
        ...
        //slack
        if(module_is_active('Slack') && !empty(company_setting('New Task Comment')) && company_setting('New Task Comment')  == true)
        {
        ...
        // telegram
        if(module_is_active('Telegram') && !empty(company_setting('Telegram New Task Comment')) && company_setting('Telegram New Task Comment')  == true)
        {
            $msg = "comment added in " . $task->title . ".";
            // SendTelegramMsg
            event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
        ...
        if(module_is_active('Webhook')){
            ...
            $comments->task_id = $task->title;
            $action = 'New Task Comment';
            $module = 'Taskly';
            event(new \Modules\Webhook\Events\SendWebhook($module ,$comments,$action));
        }
    ...
    public function bugReportStore(Request $request,$project_id)
    {
        ...
        if($project)
        {
            ...
            // twilio
            if(module_is_active('Twilio') && !empty(company_setting('Twilio New Bug')) && company_setting('Twilio New Bug')  == true)
            {
                $Assign_user_phone = User::where('id',$request->assign_to)->first();
                if(!empty($Assign_user_phone->mobile_no))
                {
                    $msg = "New Bug ".$request->title . " of " . $project->name . " created by " . Auth::user()->name . '.';
                    event(new \Modules\Twilio\Events\SendTwilioMsg($Assign_user_phone->mobile_no,$msg));
                }
            }
