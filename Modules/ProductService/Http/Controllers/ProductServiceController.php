...
class ProductServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(Auth::user()->can('product&service create'))
        {
            ...
            if(module_is_active('Webhook')){
                if($request->tax_id)
                {
                    $Tax = Tax::whereIn('id',$request->tax_id)->get()->pluck('name')->toArray();
                    if(count($Tax) > 0)
                    {
                        $tax_name = implode(',',$Tax);
                    }
                    $productService->tax_id = $tax_name;
                }
                if($request->unit_id)
                {
                    $unit = Unit::where('id',$request->unit_id)->first();
                    $productService->unit_id = $unit->name;
                }
                if($request->category_id)
                {
                    $category     = Category::where('id',$request->category_id)->where('type', '=', 0)->first();
                    $productService->category_id = $category->name;
                }
                $action = 'New Product';
                $module = 'ProductService';
                event(new \Modules\Webhook\Events\SendWebhook($module ,$productService,$action));
            }
            if(module_is_active('CustomField'))
            {
    ...
    {
        return redirect()->route('product-service.index')->with('error', __('Permission denied.'));
