                            ...
                            r ? dragula(n, {
                                ...
                                $.ajax({
                                    ...
                                    success: function (data) {
                                        toastrs('{{__("Success")}}', 'Deal Move Successfully!', 'success');
                                    },
                                    error: function (data) {
...
<div class="col-auto pe-0 pt-2 px-1">
    <a href="{{ route('deals.list') }}" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('List View')}}" class="btn btn-sm btn-primary btn-icon"><i class="ti ti-list"></i> </a>
</div>
@can('deal create')
<div class="col-auto ps-1 pt-2">
    <a class="btn btn-sm btn-primary btn-icon" data-bs-toggle="tooltip" data-bs-placement="top" title="{{__('Create Deal')}}" data-ajax-popup="true" data-size="md" data-title="{{__('Create Deal')}}" data-url="{{route('deals.create')}}"><i class="ti ti-plus text-white"></i></a>
</div>
@endcan
                                    ...
                                    @foreach($deals as $deal)
                                        <div class="card" data-id="{{$deal->id}}">
                                                ...
                                                <div class="card-header border-0 pb-0 position-relative">
                                                    <h5><a href="@can('deal show') @if($deal->is_active) {{route('deals.show',$deal->id)}} @else # @endif @else # @endcan" class="text-body text-primary">{{$deal->name}} </a></h5>
                                                            ...
                                                            <div class="btn-group card-option">
                                                                ...
                                                                @else
                                                                    ...
                                                                    <div class="dropdown-menu dropdown-menu-end">
                                                                         @can('deal edit')
                                                                                <a data-url="{{ URL::to('deals/'.$deal->id.'/labels') }}" data-ajax-popup="true" data-title="{{__('Labels')}}" class="dropdown-item">{{__('Labels')}}</a>
                                                                                <a data-url="{{ URL::to('deals/'.$deal->id.'/edit') }}" data-size="lg" data-ajax-popup="true" data-title="{{__('Edit Deal')}}" class="dropdown-item">{{__('Edit')}}</a>
                                                                            @endcan
                                                                            @can('deal delete')
                                                ...
                                                <div class="card-body">
                                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                                        <ul class="list-inline mb-0">
                                                            <li class="list-inline-item d-inline-flex align-items-center" data-bs-toggle="tooltip" data-bs-original-title="Product"><i class="f-16 text-primary ti ti-list"></i>{{count($deal->tasks)}}/{{count($deal->complete_tasks)}}
                                                            </li>
                                                        </ul>
                                                        <div class="user-group">
                                                            <i class="text-primary ti ti-report-money"></i> {{ currency_format_with_sym($deal->price)}}
                                                        </div>
                                                    </div>

                                                    <div class="d-flex align-items-center justify-content-between">
                                                        <ul class="list-inline mb-0">
                                                            <li class="list-inline-item d-inline-flex align-items-center" data-bs-toggle="tooltip" data-bs-original-title="Product"><i class="f-16 text-primary ti ti-shopping-cart"></i>{{count($deal->products())}}
                                                            </li>
                                                            <li class="list-inline-item d-inline-flex align-items-center" data-bs-toggle="tooltip" data-bs-original-title="Source"><i class="f-16 text-primary ti ti-social"></i>{{count($deal->sources())}}
                                                            </li>
                                                        </ul>
                                                        <div class="user-group">
                                                            @foreach($deal->users as $user)
                                                            <img alt="image" data-bs-toggle="tooltip" data-bs-original-title="{{$user->name}}" data-bs-placement="top" aria-label="{{$user->name}}" title="{{$user->name}}" @if($user->avatar) src="{{get_file($user->avatar)}}" @else src="{{ get_file('uploads/users-avatar/avatar.png')}}" @endif class="rounded-circle " width="25" height="25">
                                                            @endforeach
