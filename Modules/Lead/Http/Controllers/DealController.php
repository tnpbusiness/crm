    ...
    public function store(Request $request)
    {
        ...
        if ($usr->can('deal create')) {
            ...
            if (empty($stage)) {
                ...
                // slack
                if (module_is_active('Slack') && !empty(company_setting('New Deal')) && company_setting('New Deal')  == true) {
                    $msg = 'New Deal created by the ' . \Auth::user()->name . '.';
                    // SendSlackMsg
                    event(new \Modules\Slack\Events\SendSlackMsg($msg));
                }

                // telegram
                if (module_is_active('Telegram') && !empty(company_setting('Telegram New Deal')) && company_setting('Telegram New Deal')  == true) {
                    $msg = 'New Deal created by the ' . \Auth::user()->name . '.';
                    // SendTelegramMsg
                ...
                // twilio
                if (module_is_active('Twilio') && !empty(company_setting('Twilio New Deal')) && company_setting('Twilio New Deal')  == true) {
                ...
                if(module_is_active('Webhook')){
                    ...
                    $deal->pipeline_id = $pipeline->name;
                    $deal->stage_id     = $stage->name;
                    $deal->clients     = (count($clients_name) > 0 ) ? implode(',',$clients_name) : 'Not Found';
                    $action = 'New Deal';
                    $module = 'Lead';
                    event(new \Modules\Webhook\Events\SendWebhook($module ,$deal,$action));
                }
                $resp = null;
    ...
    public function order(Request $request)
    {
        ...
        if ($usr->can('deal move')) {
            ...
            if ($deal->stage_id != $post['stage_id']) {
                ...
                // slack
                if (module_is_active('Slack') && !empty(company_setting('Deal Moved')) && company_setting('Deal Moved')  == true) {
                    $msg = $deal->name . " status changed from " . $deal->stage->name . " to " . $newStage->name . '.';
                    // SendSlackMsg
                    event(new \Modules\Slack\Events\SendSlackMsg($msg));
                }

                // telegram
                if (module_is_active('Telegram') && !empty(company_setting('Telegram Deal Moved')) && company_setting('Telegram Deal Moved')  == true) {
                    $msg = $deal->name . " status changed from " . $deal->stage->name . " to " . $newStage->name . '.';
                    // SendTelegramMsg
                    event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
                }
                ...
                // twilio
                if (module_is_active('Twilio') && !empty(company_setting('Twilio Deal Moved')) && company_setting('Twilio Deal Moved')  == true) {
                ...
                if(module_is_active('Webhook')){
                    ...
                    $action = 'Deal Moved';
                    $module = 'Lead';
                    event(new \Modules\Webhook\Events\SendWebhook($module ,$deal_stages,$action));
                }
        ...
        if ($usr->can('deal task create')) {
            ...
            if ($deal->created_by == creatorId() && $deal->workspace_id == getActiveWorkSpace()) {
                ...
                if(module_is_active('Calender'))
                {
                    ...
                    {
                        $type ='task';
                        $dealTask->title=$request->name;
                        $dealTask->start_date=$request->date;
                        $dealTask->end_date=$request->date;
                        event(new \Modules\Calender\Events\GoogleCalender($dealTask,$type));
                    }
                }
