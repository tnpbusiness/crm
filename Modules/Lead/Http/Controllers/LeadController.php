...
class LeadController extends Controller
{
        ...
        if ($usr->can('lead create')) {
            ...
            if (empty($stage)) {
                ...
                if(module_is_active('Calender'))
                {
                    ...
                    {
                        $type ='holiday';
                        $lead->title=$request->occasion;
                        event(new \Modules\Calender\Events\GoogleCalender($lead,$type));
                    }
                }
                // slack
                if (module_is_active('Slack') && !empty(company_setting('New Lead')) && company_setting('New Lead')  == true) {
                    $msg = 'New lead created by the ' . Auth::user()->name . '.';
                    event(new \Modules\Slack\Events\SendSlackMsg($msg));
                }

                //telegram
                if (module_is_active('Telegram') && !empty(company_setting('Telegram New Lead')) && company_setting('Telegram New Lead')  == true) {
                    $msg = 'New lead created by the ' . Auth::user()->name . '.';
                    // SendTelegramMsg
                ...
                // twilio
                if (module_is_active('Twilio') && !empty(company_setting('Twilio New Lead')) && company_setting('Twilio New Lead')  == true) {
                ...
                if(module_is_active('Webhook')){
                    ...
                    $lead->pipeline_id = $pipeline->name;
                    $lead->user_id     = $user->name;
                    $lead->stage_id     = $stage->name;
                    $action = 'New Lead';
                    $module = 'Lead';
                    event(new \Modules\Webhook\Events\SendWebhook($module ,$lead,$action));
                }
                $resp = null;
    ...
    public function order(Request $request)
    {
        ...
        if (Auth::user()->can('lead move')) {
            ...
            if ($lead->stage_id != $post['stage_id']) {
                ...
                // slack
                if (module_is_active('Slack') && !empty(company_setting('Lead Moved')) && company_setting('Lead Moved')  == true) {
                    $msg = $lead->name . " status changed from  " . $lead->stage->name . " to " . $newStage->name . '.';
                    // SendSlackMsg
                    event(new \Modules\Slack\Events\SendSlackMsg($msg));
                }

                // telegram
                if (module_is_active('Telegram') && !empty(company_setting('Telegram Lead Moved')) && company_setting('Telegram Lead Moved')  == true) {
                    $msg = $lead->name . " status changed from  " . $lead->stage->name . " to " . $newStage->name . '.';
                    // SendTelegramMsg
                    event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
                }
                ...
                // twilio
                if (module_is_active('Twilio') && !empty(company_setting('Twilio Lead Moved')) && company_setting('Twilio Lead Moved')  == true) {
                    $Assign_user_phone = User::where('id', $request->user_id)->first();
                ...
                if(module_is_active('Webhook')){
                    ...
                    $lead_stages->stage_id = $newStage->name;
                    $lead_stages->order = $lead->order;
                    $action = 'Lead Moved';
                    $module = 'Lead';
                    event(new \Modules\Webhook\Events\SendWebhook($module ,$lead_stages,$action));
                }
    ...
    public function convertToDeal($id, Request $request)
    {
        ...
        // slack
        if (module_is_active('Slack') && !empty(company_setting('Lead to Deal Conversion')) && company_setting('Lead to Deal Conversion')  == true) {
        ...
        // telegram
        if (module_is_active('Telegram') && !empty(company_setting('Telegram Lead to Deal Conversion')) && company_setting('Telegram Lead to Deal Conversion')  == true) {
            $msg = 'Deal converted through lead' . $lead->name . '.';
            // SendTelegramMsg
            event(new \Modules\Telegram\Events\SendTelegramMsg($msg));
        }
        ...
        // twilio
        if (module_is_active('Twilio') && !empty(company_setting('Twilio Lead to Deal Conversion')) && company_setting('Twilio Lead to Deal Conversion')  == true) {
