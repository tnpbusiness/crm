...
Route::group(['middleware' => ['verified']], function ()
{
    ...
    Route::group(['middleware' => 'PlanModuleCheck:Account-Taskly'], function ()
    {
        ...
        Route::post('tax/detail/get', 'InvoiceController@TaxDetailGet')->name('tax.detail.get')->middleware(
            [
                'auth',
            ]
        );
        ...
        Route::post('taxs/detail/get', 'ProposalController@TaxDetailGet')->name('tax.details.gets')->middleware(
            [
                'auth',
            ]
        );
        Route::get('proposal/{id}/sent', 'ProposalController@sent')->name('proposal.sent');
        Route::post('proposal/get_tax', 'ProposalController@getTax')->name('get_tax');
    });
});
